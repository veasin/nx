...
trait route{
	protected function nx_router_route(){
		if(!isset($this->buffer['router/route'])) $this->buffer['router/route']=isset($this->setup['router/route'])
			?$this->setup['router/route']
			:['rules'=>[]];
	}
	/**
	 * 默认路由方法 循环匹配路由
	...
	public function router(){
		...
		$uri=(isset($_SERVER['PATH_INFO']) && !empty($_SERVER['PATH_INFO'])) ?ltrim($_SERVER['PATH_INFO'], '/')
