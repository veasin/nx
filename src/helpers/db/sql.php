 ...
 * ?@method static sql\part null_safe_equal($N1, $N2) <=>
 * ?@method static sql\part no_equal($N1, $N2) !=
 * ?@method static sql\part lt_equal($N1, $N2) <=
 * ?@method static sql\part lt($N1, $N2) <
 * ?@method static sql\part gt_equal($N1, $N2) >=
 * ?@method static sql\part gt($N1, $N2) >
 * @method static sql\part is($expr) 根据一个布尔值来检验一个值，在这里，布尔值可以是TRUE、FALSE或UNKNOWN
 * @method static sql\part isNot($expr)
 * @-method static sql\part isNull($expr) IS NULL IS NOT NULL 检验一个值是否为 NULL
 * @method static sql\part isNotNull($expr) IS NULL IS NOT NULL 检验一个值是否为 NULL
 ...
 * @method static sql\part ISNULL($expr) 如expr 为NULL，那么ISNULL() 的返回值为 1，否则返回值为 0
 * @method static sql\part INTERVAL($N,...$Ns) 假如N < N1，则返回值为0；假如N < N2 等等，则返回值为1；假如N 为NULL，则返回值为 -1 。所有的参数均按照整数处理。为了这个函数的正确运行，必须满足 N1 < N2 < N3 < ……< Nn 。其原因是使用了二分查找(极快速)
 ...
 * @method static sql\part and($expr) AND && 逻辑AND。当所有操作数均为非零值、并且不为NULL时，计算所得结果为  1 ，当一个或多个操作数为0 时，所得结果为 0 ，其余情况返回值为 NULL
 * @method static sql\part or($expr) OR || 逻辑 OR。当两个操作数均为非 NULL值时，如有任意一个操作数为非零值，则结果为1，否则结果为0。当有一个操作数为NULL时，如另一个操作数为非零值，则结果为1，否则结果为 NULL 。假如两个操作数均为  NULL，则所得结果为 NULL
 * @method static sql\part xor($expr) XOR 逻辑XOR。当任意一个操作数为 NULL时，返回值为NULL。对于非   NULL 的操作数，假如一个奇数操作数为非零值，则计算所得结果为  1 ，否则为  0
 * 12.2. 控制流程函数
 ...
 * @method static sql\part CHARSET($string)
 ...
 * @method static sql\part COMPRESS($string) 压缩一个字符串。这个函数要求 MySQL已经用一个诸如zlib的压缩库压缩过。   否则，返回值始终是NULL。UNCOMPRESS() 可将压缩过的字符串进行解压缩
 ...
 * @method static sql\part CONV($number, $fromBase, $toBase) 不同数基间转换数字。返回值为数字的N字符串表示，由from_base基转化为 to_base 基。如有任意一个参数为NULL，则返回值为 NULL。自变量 N 被理解为一个整数，但是可以被指定为一个整数或字符串。最小基数为 2 ，而最大基数则为 36。 If to_base 是一个负数，则 N 被看作一个带符号数。否则， N 被看作无符号数。 CONV() 的运行精确度为 64比特
 ...
 * @method static sql\part FORMAT($x, $d) 将number X设置为格式 '#,###,###.##', 以四舍五入的方式保留到小数点后D位, 而返回结果为一个字符串
 ...
 * @method static sql\part LIKE($expr,$pat,$ESCAP=null) 模式匹配，使用SQL简单正规表达式比较。返回1 (TRUE) 或 0 (FALSE)。 若 expr 或 pat 中任何一个为 NULL,则结果为 NULL
 * ?@method static sql\part NOLIKE($expr,$pat,$ESCAP=null) 模式匹配，使用SQL简单正规表达式比较。返回1 (TRUE) 或 0 (FALSE)。 若 expr 或 pat 中任何一个为 NULL,则结果为 NULL
 * ?@method static sql\part REGEXP($expr,$pat) 模式匹配，使用SQL简单正规表达式比较。返回1 (TRUE) 或 0 (FALSE)。 若 expr 或 pat 中任何一个为 NULL,则结果为 NULL
 * ?@method static sql\part NOTREGEXP($expr,$pat) 模式匹配，使用SQL简单正规表达式比较。返回1 (TRUE) 或 0 (FALSE)。 若 expr 或 pat 中任何一个为 NULL,则结果为 NULL
 ...
 * @method static sql\part CEIL($X) 返回不小于X 的最小整数值
 ...
 * @-method static sql\part FORMAT($X) 将数字X 的格式写成'#,###,###.##'格式, 即保留小数点后 D位，而第D位的保留方式为四舍五入，然后将结果以字符串的形式返回。详见12.9.4节，“其他函数”
 ...
 * @method static sql\part CURRENT_DATE() 将当前日期按照'YYYY-MM-DD' 或YYYYMMDD 格式的值返回，具体格式根据函数用在字符串或是数字语境中而定
 * @method static sql\part CURTIME() 将当前时间以'HH:MM:SS'或 HHMMSS 的格式返回， 具体格式根据函数用在字符串或是数字语境中而定
 * @method static sql\part CURRENT_TIME() 将当前时间以'HH:MM:SS'或 HHMMSS 的格式返回， 具体格式根据函数用在字符串或是数字语境中而定
 * @method static sql\part CURRENT_TIMESTAMP() 是NOW的同义词
 ...
 * @method static sql\part DATE_ADD($date) 这些函数执行日期运算。 date 是一个 DATETIME 或DATE值，用来指定起始时间。 expr 是一个表达式，用来指定从起始日期添加或减去的时间间隔值。  Expr是一个字符串;对于负值的时间间隔，它可以以一个 ‘-’开头。 type 为关键词，它指示了表达式被解释的方式
 * @method static sql\part DATE_FORMAT($date,$format) 根据format 字符串安排date 值的格式
 * @method static sql\part DAY($date) 和DAYOFMONTH()的意义相同
 ...
 * @method static sql\part LOCALTIME() 和NOW具有相同意义
 * @method static sql\part LOCALTIMESTAMP() 和NOW具有相同意义
 ...
 * @method static sql\part OLD_PASSWORD($str) 当PASSWORD()的执行变为改善安全性时，OLD_PASSWORD()会被添加到 MySQL。OLD_PASSWORD()返回从前的PASSWORD()执行值( 4.1之前)，同时允许你为任何4.1 之前的需要连接到你的5.1 版本MySQL服务器前客户端设置密码，从而不至于将它们切断
 * @method static sql\part PASSWORD($str) 从原文密码str 计算并返回密码字符串，当参数为 NULL 时返回 NULL。这个函数用于用户授权表的Password列中的加密MySQL密码存储
 ...
 * @method static sql\part SCHEMA() 这个函数和 DATABASE具有相同的意义
 * @method static sql\part SESSION_USER() 和 USER()具有相同的意义
 * @method static sql\part SYSTEM_USER() 和 USER()具有相同的意义
 ...
 * @-method static sql\part FORMAT($X,$D) 将数字X 的格式写为'#,###,###.##',以四舍五入的方式保留小数点后 D 位， 并将结果以字符串的形式返回。若  D 为 0, 则返回结果不带有小数点，或不含小数部分
 * @method static sql\part GET_LOCK($str,$timeout) 设法使用字符串str 给定的名字得到一个锁， 超时为timeout 秒。若成功得到锁，则返回 1，若操作超时则返回0  (例如,由于另一个客户端已提前封锁了这个名字 ),若发生错误则返回NULL (诸如缺乏记忆或线程mysqladmin kill 被断开 )。假如你有一个用GET_LOCK()得到的锁，当你执行RELEASE_LOCK()或你的连接断开(正常或非正常)时，这个锁就会解除。
 ...
 * @method static sql\part IS_FREE_LOCK($str) 检查名为str的锁是否可以使用 (换言之,没有被封锁)。若锁可以使用，则返回  1  (没有人在用这个锁), 若这个锁正在被使用，则返回0 ，出现错误则返回 NULL  (诸如不正确的参数 )
 * @method static sql\part IS_USED_LOCK($str) 检查名为str的锁是否正在被使用(换言之,被封锁)。若被封锁，则返回使用该锁的客户端的连接标识符。否则返回 NULL
